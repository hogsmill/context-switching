      ...
      <div class="container">
        <div class="row">
          <div class="col no-switching">
            ...
            <h3>
              {{ getTime(contexts.noSwitching) }} <button id="go-no-switch" class="btn btn-info" :disabled="currentContext == 'switching'" @click="goNoSwitch($event)">
                Go
              </button>
            </h3>
            ...
            <div class="row">
              ...
              >
                ...
                <input type="text" disabled="true" tabindex="-1"
                       ...
                       @keyup.enter="enter($event, topicIndex)"
                >
          ...
          <div class="col switching">
            ...
            <h3>
              {{ getTime(contexts.switching) }} <button id="go-switch" class="btn btn-info" :disabled="currentContext == 'noSwitching'" @click="goSwitch($event)">
                Go
              </button>
            </h3>
...
export default {
  ...
  methods: {
    ...
    getTime: function(context) {
      let minutes = parseInt(context.time / 60)
      let seconds = context.time - (minutes * 60)
    ...
    getTopics: function() {
      const topics = []
      const l = this.topicsList.length
      while (topics.length < 3) {
        const rand = Math.floor(Math.random() * l)
        const topic = this.topicsList[rand]
        let dupe = false
    ...
    _goNoSwitch: function(time) {
      ...
      if (this.time >= 60) {
        ...
        const rem = this.time % this.timePerTopic
        if (rem == 0) {
          for (let i = 0; i < this.topics; i++) {
            this.setTopicActive(i, false)
          }
          const col = this.time / this.timePerTopic
          this.setTopicActive(col, true)
        }
      }
      if (this.starterNoSwitch && this.time < 60) {
        const data = {context: this.currentContext, time: this.time + 1}
        const self = this
